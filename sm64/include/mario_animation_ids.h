/*
 * new64 -- animation identifiers.
 *
 * new64 ships no animation data, so these are pure identifiers: a request to
 * play one is recorded on the player and exposed to the renderer and the trace,
 * which is enough for a capsule.
 *
 * They are not cosmetic, though.  Some original actions end when their
 * animation ends, so is_anim_at_end() has to return TRUE on the right frame or
 * those actions run forever.  new64 solves this with a timing table
 * (assets/mario_anims.txt) giving each id a frame count; the shipped table
 * carries reasonable defaults, and docs/SLOTTING_IN_DECOMP.md explains how to
 * replace it with real values extracted from an asset dump.
 */
#ifndef NEW64_MARIO_ANIMATION_IDS_H
#define NEW64_MARIO_ANIMATION_IDS_H
...
enum MarioAnimID {
    ...
    MARIO_ANIM_STOP_CRAWLING_WITH_LIGHT_OBJ,
    MARIO_ANIM_START_CRAWLING_WITH_LIGHT_OBJ,
    MARIO_ANIM_CRAWLING_WITH_LIGHT_OBJ,
    ...
    MARIO_ANIM_BEND_KNEES_RIDING_SHELL,
    ...
    MARIO_ANIM_SIDESTEP_LEFT_UNUSED,
    MARIO_ANIM_SIDESTEP_RIGHT_UNUSED,
    MARIO_ANIM_START_SLEEP_SITTING,
    MARIO_ANIM_SLEEP_SITTING,
    MARIO_ANIM_STAND_UP_FROM_LYING,
    MARIO_ANIM_WALL_KICK_AIR,
    ...
    MARIO_ANIM_STAND_UP_FROM_FAST_LONGJUMP,
    MARIO_ANIM_FALL_FROM_SLIDE_KICK_UNUSED,
    MARIO_ANIM_FALL_FROM_FAST_LONGJUMP,
    MARIO_ANIM_SINGLE_JUMP_UNUSED,
...
#endif /* NEW64_MARIO_ANIMATION_IDS_H */
